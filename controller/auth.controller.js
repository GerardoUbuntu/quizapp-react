...
module.exports = {
    signUp: async (req, res) => {
        ...
        if (email) return res.status(status.INTERNAL_SERVER_ERROR).json({ msg: `User with email ${user.email} already exists` });
    ...
    signIn: async (req, res) => {
        ...
        findBy('email', email)
            .then((user) => {
                if (!user) return res.status(status.INTERNAL_SERVER_ERROR).json({ msg: 'User Does not exist' });

                // Validate password
                bcrypt.compare(password, user.password).then((isMatch) => {
                    if (!isMatch) return res.status(status.INTERNAL_SERVER_ERROR).json({ msg: 'Invalid Credentials' });
                    jwt.sign(
                        { id: user.id },
                        process.env.JWT_SECRET,
                        {
